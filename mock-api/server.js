...
app.post('/auth/register', (req, res) => {
  ...
  const user = { id: randomUUID(), name, username, email, password, created_at: now() };
...
app.post('/auth/login', (req, res) => {
  ...
  const user = db().get('users').find(u => (u.email === emailOrUsername || u.username === emailOrUsername) && u.password === password).value();
  if (!user) return res.status(401).json({ error: 'unauthorized' });
...
app.get('/users/lookup', requireAuth, (req, res) => {
  ...
  if (!identifier) {
    return res.status(400).json({ error: 'identifier query param is required' });
  }

  const user = db().get('users').find(u => u.email === identifier || u.username === identifier || u.id === identifier).value();
  if (!user) {
...
app.get('/projects', requireAuth, (req, res) => {
  ...
  const projects = db()
    ...
    .filter(
      (p) =>
        p.owner_id === userId ||
        (p.contributor_ids ?? []).includes(userId),
    )
    .value();
...
app.delete('/projects/:id/columns/:columnId', requireAuth, (req, res) => {
  ...
  const existingColumns = project.columns ?? [...DEFAULT_COLUMNS];
  const targetColumn = existingColumns.find((c) => c.id === req.params.columnId);
  ...
  db()
    .get('projects')
    .find({ id: req.params.id })
    .assign({ columns: existingColumns.filter((c) => c.id !== req.params.columnId) })
    .write();
...
app.post('/projects/:id/contributors', requireAuth, (req, res) => {
  ...
  const targetUser = db().get('users').find(u => u.email === identifier || u.username === identifier || u.id === identifier).value();
  if (!targetUser) {
    return res.status(404).json({
      error: 'no_account',
...
app.delete('/projects/:id/contributors/:userId', requireAuth, (req, res) => {
  ...
  db()
    ...
    .find({ id: req.params.id })
    .assign({
      contributor_ids: existingContributorIds.filter((id) => id !== req.params.userId),
    })
    .write();
...
app.patch('/tasks/:id', requireAuth, (req, res) => {
  ...
  if (updateData.assignee_ids !== undefined && !Array.isArray(updateData.assignee_ids)) {
  ...
  return res.json(normalizeTask(db().get('tasks').find({ id: req.params.id }).value()));
});
